...
class ManifestManager {
   ...
   * Handles race conditions by queuing rewrites when called during pending write
   */
  async writeManifest() {
    if (this.isPending) {
      // Flag that we need to write again after the current write completes
      this.needsRewrite = true;
      console.log('[ManifestManager] Write already pending, will rewrite after completion');
      return;
    }

    try {
      // Loop to handle any writes that were requested during our write
      do {
        this.needsRewrite = false;
        this.isPending = true;
        ...
        // Modern encoding: TextEncoder -> Uint8Array -> base64 (replaces deprecated unescape())
        ...
        this.isPending = false;
      } while (this.needsRewrite); // Rewrite if another append happened during this write
    } catch (error) {
