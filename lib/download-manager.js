...
class DownloadManager {
  ...
  async handleCompletion() {
    if (this.activeDownload && this.activeDownload.promise) {
      ...
      // Record download in manifest
      await this.recordDownloadInManifest();
  ...
  /**
   * Record the completed download in the manifest
   */
  async recordDownloadInManifest() {
    try {
      // Only record if we have metadata and a download ID
      if (!this.activeDownload?.downloadId || !this.activeDownload?.metadata) {
        return;
      }

      const downloadId = this.activeDownload.downloadId;
      const metadata = this.activeDownload.metadata;

      // Query Chrome to get the final filename (includes uniquify suffix)
      const downloads = await chrome.downloads.search({ id: downloadId });
      if (downloads.length === 0 || !downloads[0].filename) {
        console.warn('[DownloadManager] Could not find download file path for manifest');
        return;
      }

      const fullPath = downloads[0].filename;
      
      // Extract relative path within Downloads folder
      // fullPath is typically like "/Users/.../Downloads/TrailMix/Artist/Album/file.zip"
      let relativePath = fullPath;
      const trailMixIndex = fullPath.indexOf('TrailMix');
      if (trailMixIndex !== -1) {
        relativePath = fullPath.substring(trailMixIndex);
      }

      // Get manifest manager (should be available globally in service worker)
      if (typeof manifestManager !== 'undefined') {
        ...
        console.log('[DownloadManager] Recorded download in manifest:', relativePath);
      } else {
        console.warn('[DownloadManager] manifestManager not available');
      }
    } catch (error) {
      // Don't let manifest errors break the download flow
      console.error('[DownloadManager] Failed to record download in manifest:', error);
    }
  }
